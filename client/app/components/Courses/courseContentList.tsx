    ...
    >
      {videoSections.map((section: string, sectionIndex: number) => {
        ...
        return (
          <div
            className={`${
              !props.isDemo && "border-b border-[#0000001c] dark:border-[#ffffff8e] pb-2"
            }`}
            key={section}
          >
            ...
            {isSectionVisible && (
              <div className="w-full">
                {sectionVideos.map((item: any, index: number) => {
                  ...
                  const contentLength: number = item.videoDuration / 60;
                  return (
                    <div
                      className={`w-full text-black dark:text-white ${
                        videoIndex === props.activeVideo ? "bg-slate-800 text-white" : ""
                    ...
                    >
                      <div className="flex items-start">
                        ...
                        <h1 className="text-[18px] inline-block break-words ">{item.title}</h1>
                      </div>
                      <h5 className="pl-8 ">
                        {item.videoDuration > 60 ? contentLength.toFixed(2) : item.videoDuration}{" "}
